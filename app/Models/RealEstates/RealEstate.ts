...
export default class RealEstate extends BaseModel {
  ...
  @column()
  public landCadastralNumber: string | undefined;
  ...
  public get title(): string {
    ...
    if (this.roomType == 0) room = "Студия";
    else if (this.roomType == 6) room = "5+ - к";
    else if (this.roomType == 7) room = "Свободная планировка";
    else room = `${this.roomType}-к`;
  ...
  public static createUuid(realEstate: RealEstate) {
    if (!realEstate.uuid) realEstate.uuid = uuid();
  }

  @beforeSave()
  public static setCommissionAndAddress(realEstate: RealEstate) {
    if (realEstate.commission > 100) realEstate.commission = 100;
  ...
  public static async preloadRelations(
    query: ModelQueryBuilderContract<typeof RealEstate>
  ) {
    query.preload("user").preload("images").preload("district");
  }
  ...
  public async getForUser(currentUserId: User["id"]): Promise<ModelObject> {
    ...
    try {
      await (this as RealEstate)
        .related("realEstatesViews")
        .attach([currentUserId]);
    } catch (err: any) {}

    const isInWishlist = await Database.from("realEstatesWishlists")
      .where("user_id", currentUserId)
      .andWhere("realEstate_id", item.id)
      .first();

    const isInReports = await Database.from("realEstatesReports")
      .where("user_id", currentUserId)
      .andWhere("realEstate_id", item.id)
      .first();

    const isViewed = await Database.from("realEstatesViews")
      .where("user_id", currentUserId)
      .andWhere("realEstate_id", item.id)
      .first();
